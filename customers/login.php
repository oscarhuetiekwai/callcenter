...
if (isset($_GET['acc_user']) AND isset($_GET['acc_pwd'])) {
mysql_select_db($database_contacts, $contacts);
$query_logincheck = "SELECT * FROM users WHERE user_name = '".addslashes($_GET['acc_user'])."' AND user_password = '".addslashes($_GET['acc_pwd'])."'";
...
if ($totalRows_logincheck==1) { 
    $_SESSION['accordia'] = true;
	$_SESSION['user'] = addslashes($row_logincheck['user_email']);
